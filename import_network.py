	...
	def execute(self, context):		   # execute() is called when running the operator.
		...
		with open(edatapath) as csvfile:
			...
			for i, row in enumerate( rdr ):
				...
				efromname,etoname,esz, ecol,from_x, from_y, from_z, to_x, to_y, to_z, ered,egreen,eblue,ecurve,fromshort,toshort,arrowlength,arrowsize,edge3d,edash = [row[i] for i in ecolind] 
				...
				if ename not in edges.objects:	
					...
					if(float(edash)==0):
						...
						curve.materials.append(bpy.data.materials[ecol+edash])
					curve.dimensions = '3D'
